...
# Stored Procedure para agregar películas por parámetros
DELIMITER $$
CREATE PROCEDURE `sp_get_add_films`(IN name VARCHAR(200), IN year INT, IN director_id INT, 
IN actor_id INT, IN genre_id INT, IN country_id INT)
BEGIN
	SET @film_data = concat("'", name, "','", year, "','",director_id, "','",actor_id, 
    "','",genre_id, "','",country_id, "')");
